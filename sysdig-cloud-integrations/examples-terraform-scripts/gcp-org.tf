...
module "organization-posture" {
  ...
  service_account_name = "sysdig-secure-q6re"
  is_organizational    = true
  organization_domain  = "organizatin.domain"
}
...
resource "sysdig_secure_cloud_auth_account" "gcp_project_[Target GCP Project ID]" {
  ...
  depends_on = [module.organization-posture, module.organization-threat-detection, module.vm-host-scan]
}

resource "sysdig_secure_organization" "gcp_organization_[Target GCP Project ID]" {
  organizational_unit_ids = []
  management_account_id   = sysdig_secure_cloud_auth_account.gcp_project_[Target GCP Project ID].id
  depends_on              = [module.organization-posture, module.organization-threat-detection, module.vm-host-scan]
}
